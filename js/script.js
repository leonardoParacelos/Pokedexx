...
const renderPokemon = async (pokemon) => {
  ...
  if (data) {
    ...
    if (data.id >= 650 && data.id <= 721) {
      ...
      document.querySelector('.pokemon_image').style.width = '10%';
      document.querySelector('.pokemon_image').style.height = '15%';
      document.querySelector('.pokemon_image').style.bottom = '50%';
    }

    if (data.id >= 721 && data.id <= 809) {
      pokemonImage.src = data['sprites']['versions']['generation-vii']['icons']['front_default'];
      document.querySelector('.pokemon_image').style.width = '16%';
      document.querySelector('.pokemon_image').style.height = '14%';
      document.querySelector('.pokemon_image').style.bottom = '50%';
    }
    ...
    if (data.id >= 809 && data.id <= 905) {
      ...
      document.querySelector('.pokemon_image').style.width = '16%';
      document.querySelector('.pokemon_image').style.height = '14%';
      document.querySelector('.pokemon_image').style.bottom = '49.5%';
    }
